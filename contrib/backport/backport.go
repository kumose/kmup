// Copyright 2023 The Kmup Authors. All rights reserved.
...
func main() {
	...
	app.Description = `Backport will look-up the PR in Kmup's git log and attempt to cherry-pick it on the current version`
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:  "upstream",
			Value: "origin",
			Usage: "Upstream remote for the Kmup upstream",
		},
		&cli.StringFlag{
			Name:  "release-branch",
			Value: "",
		...
		&cli.StringFlag{
			Name:  "remote",
			Value: "",
			Usage: "Remote for your fork of the Kmup upstream",
		},
		&cli.StringFlag{
			Name:  "fork-user",
			Value: "",
...
func runBackport(ctx context.Context, c *cli.Command) error {
	...
	if !c.Bool("no-push") {
		url := "https://github.com/kumose/kmup/compare/" + upstreamReleaseBranch + "..." + forkUser + ":" + backportBranch
...
func fetchRemoteAndMain(ctx context.Context, remote, releaseBranch string) error {
	fmt.Printf("* `git fetch %s master`\n", remote)
	out, err := exec.CommandContext(ctx, "git", "fetch", remote, "master").Output()
	if err != nil {
		fmt.Println(string(out))
		return fmt.Errorf("unable to fetch %s from %s: %w", "master", remote, err)
	}
	fmt.Println(string(out))
...
func determineRemote(ctx context.Context, forkUser string) (string, string, error) {
	...
	for line := range lines {
		...
		if strings.Contains(remote, "kumose/kmup") {
			continue
		}
...
func determineSHAforPR(ctx context.Context, prStr, accessToken string) (string, error) {
	...
	pr, _, err := client.PullRequests.Get(ctx, "kumose", "kmup", prNum)
	if err != nil {
		return "", err
	}
