...
import (
	...
	"github.com/kumose/kmup/modules/templates/scopedtmpl"
...
func wrapTmplErrMsg(msg string) {
	...
	if setting.IsProd {
		// in prod mode, Kmup must have correct templates to run
		log.Fatal("Kmup can't run with template errors: %s", msg)
	}
	...
	log.Error("There are template errors but Kmup continues to run in dev mode: %s", msg)
}
