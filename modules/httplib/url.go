...
const (
	urlTypeKmupAbsolute     urlType = iota + 1 // "http://kmup/subpath"
	urlTypeKmupPageRelative                    // "/subpath"
	urlTypeKmupSiteRelative                    // "?key=val"
...
func detectURLRoutePath(ctx context.Context, s string) (routePath string, ut urlType) {
	...
	if urlIsRelative(s, u) {
		if u.Path == "" {
			return "", urlTypeKmupPageRelative
		}
		if strings.HasPrefix(strings.ToLower(cleanedPath+"/"), strings.ToLower(setting.AppSubURL+"/")) {
			return cleanedPath[len(setting.AppSubURL):], urlTypeKmupSiteRelative
		}
		return "", urlTypeUnknown
	}
	u.Path = cleanedPath + "/"
	...
	if strings.HasPrefix(urlLower, strings.ToLower(setting.AppURL)) {
		return cleanedPath[len(setting.AppSubURL):], urlTypeKmupAbsolute
	}
...
func IsCurrentKmupSiteURL(ctx context.Context, s string) bool {
	_, ut := detectURLRoutePath(ctx, s)
	return ut != urlTypeUnknown
}

type KmupSiteURL struct {
	RoutePath   string
	OwnerName   string
	RepoName    string
	RepoSubPath string
...
func ParseKmupSiteURL(ctx context.Context, s string) *KmupSiteURL {
	routePath, ut := detectURLRoutePath(ctx, s)
	if ut == urlTypeUnknown || ut == urlTypeKmupPageRelative {
		return nil
	}
	ret := &KmupSiteURL{RoutePath: routePath}
