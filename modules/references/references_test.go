...
func TestConvertFullHTMLReferencesToShortRefs(t *testing.T) {
	re := regexp.MustCompile(`(\s|^|\(|\[)` +
		regexp.QuoteMeta("https://ourkmup.com/git/") +
		`([0-9a-zA-Z-_\.]+/[0-9a-zA-Z-_\.]+)/` +
		`((?:issues)|(?:pulls))/([0-9]+)(?:\s|$|\)|\]|[:;,.?!]\s|[:;,.?!]$)`)
	test := `this is a https://ourkmup.com/git/owner/repo/issues/123456789, foo
https://ourkmup.com/git/owner/repo/pulls/123456789
  And https://ourkmup.com/git/owner/repo/pulls/123
`
...
func TestFindAllIssueReferences(t *testing.T) {
	fixtures := []testFixture{
		...
		{
			"Merge pull request '#12345 My fix for a bug' (!1337) from feature-branch into master",
			[]testResult{
