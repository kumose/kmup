...
import (
	...
	"github.com/kumose/kmup/modules/markup/mdstripper"
...
var (
	...
	kmupHost             string
	kmupIssuePullPattern *regexp.Regexp
...
func getKmupHostName() string {
	kmupHostInit.Do(func() {
		if uapp, err := url.Parse(setting.AppURL); err == nil {
			kmupHost = strings.ToLower(uapp.Host)
			kmupIssuePullPattern = regexp.MustCompile(
				`(\s|^|\(|\[)` +
			...
			kmupHost = ""
			kmupIssuePullPattern = nil
		}
	})
	return kmupHost
}

// getKmupIssuePullPattern
func getKmupIssuePullPattern() *regexp.Regexp {
	getKmupHostName()
	return kmupIssuePullPattern
}
...
func convertFullHTMLReferencesToShortRefs(re *regexp.Regexp, contentBytes *[]byte) {
	...
	//
	// this is a https://ourkmup.com/git/owner/repo/issues/123456789, foo
	// https://ourkmup.com/git/owner/repo/pulls/123456789
	//
	// Into something like:
	//
	...
	pos := 0
	for {
		// re looks for something like: (\s|^|\(|\[)https://ourkmup.com/git/(owner/repo)/(issues)/(123456789)(?:\s|$|\)|\]|[:;,.?!]\s|[:;,.?!]$)
...
func FindAllIssueReferences(content string) []IssueReference {
	...
	if re := getKmupIssuePullPattern(); re != nil {
		convertFullHTMLReferencesToShortRefs(re, &contentBytes)
	} else {
		log.Debug("No KmupIssuePullPattern pattern")
	}
