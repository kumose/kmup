...
import (
	...
	"github.com/kumose/kmup/modules/git/internal" //nolint:depguard // only this file can use the internal type CmdArg, other files and packages should use AddXxx functions
...
type runOpts struct {
	...
	// * /some/path/.git
	// * /some/path/.git/kmup-data/data/repositories/user/repo.git
...
func commonBaseEnvs() []string {
	envs := []string{
		// Make Kmup use internal git config only, to prevent conflicts with user's git config
		...
		// For these users, they need to migrate the necessary configs to Kmup's git config file manually.
...
// CommonCmdServEnvs is like CommonGitCmdEnvs, but it only returns minimal required environment variables for the "kmup serv" command
