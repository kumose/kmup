...
type RepositoryURL struct {
	...
	// if the URL belongs to current Kmup instance, then the below fields have values
	OwnerName     string
	RepoName      string
	RemainingPath string
}

// ParseRepositoryURL tries to parse a Git URL and extract the owner/repository name if it belongs to current Kmup instance.
func ParseRepositoryURL(ctx context.Context, repoURL string) (*RepositoryURL, error) {
	...
	case "http", "https":
		if !httplib.IsCurrentKmupSiteURL(ctx, repoURL) {
			return ret, nil
		}
