...
var (
	// AppPath represents the path to the kmup binary
	AppPath string

	// AppWorkPath is the "working directory" of Kmup. It maps to the: WORK_PATH in app.ini, "--work-path" flag, environment variable KMUP_WORK_DIR.
	// If that is not set it is the default set here by the linker or failing that the directory of AppPath.
	// It is used as the base path for several other paths.
	AppWorkPath string
	CustomPath  string // Custom directory path. Env: KMUP_CUSTOM
	CustomConf  string
...
func InitWorkPathAndCfgProvider(getEnvFn func(name string) string, args ArgWorkPathAndCustomConf) {
	...
	readFromEnv := func() {
		envWorkPath := getEnvFn("KMUP_WORK_DIR")
		if envWorkPath != "" {
			tmpWorkPath.Set(envWorkPath)
			if !filepath.IsAbs(tmpWorkPath.Value) {
				log.Fatal("KMUP_WORK_DIR (work path) must be absolute path")
			}
		}

		envCustomPath := getEnvFn("KMUP_CUSTOM")
		if envCustomPath != "" {
			tmpCustomPath.Set(envCustomPath)
			if !filepath.IsAbs(tmpCustomPath.Value) {
				log.Fatal("KMUP_CUSTOM (custom path) must be absolute path")
			}
		}
	}
	...
	if configWorkPath != "" {
		...
		if tmpWorkPath.Value != "" && (getEnvFn("KMUP_WORK_DIR") != "" || args.WorkPath != "") {
...
// Kmup only creates subdirectories under it, but not the APP_TEMP_PATH directory itself.
// * When APP_TEMP_PATH="/tmp": the managed temp directory is "/tmp/kmup-tmp"
...
func AppDataTempDir(sub string) *tempdir.TempDir {
	if appTempPathInternal != "" {
		return tempdir.New(appTempPathInternal, "kmup-tmp/"+sub)
	}
	if AppDataPath == "" {
