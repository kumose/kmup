...
var SSH = struct {
	...
	ServerHostKeys:                []string{"ssh/kmup.rsa", "ssh/gogs.rsa"},
...
func loadSSHFrom(rootCfg ConfigProvider) {
	...
	SSH.TrustedUserCAKeysFile = sec.Key("SSH_TRUSTED_USER_CA_KEYS_FILENAME").MustString(filepath.Join(SSH.RootPath, "kmup-trusted-user-ca-keys.pem"))
