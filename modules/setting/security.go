...
var (
	...
	OnlyAllowPushIfKmupEnvironmentSet bool
...
func loadSecurityFrom(rootCfg ConfigProvider) {
	...
	CookieRememberName = sec.Key("COOKIE_REMEMBER_NAME").MustString("kmup_incredible")
	...
	OnlyAllowPushIfKmupEnvironmentSet = sec.Key("ONLY_ALLOW_PUSH_IF_KMUP_ENVIRONMENT_SET").MustBool(true)
	...
	if InstallLock && InternalToken == "" {
		// if Kmup has been installed but the InternalToken hasn't been generated (upgrade from an old release), we should generate
	...
	if sectionHasDisableQueryAuthToken && !DisableQueryAuthToken {
		log.Warn("Enabling Query API Auth tokens is not recommended. DISABLE_QUERY_AUTH_TOKEN will default to true in kmup 1.23 and will be removed in kmup 1.24.")
	}
}
