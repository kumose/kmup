...
var (
	Repository = struct {
		...
		// StreamArchives makes Kmup stream git archive files to the client directly instead of creating an archive first.
...
func loadRepositoryFrom(rootCfg ConfigProvider) {
	...
	RepoRootPath = sec.Key("ROOT").MustString(filepath.Join(AppDataPath, "kmup-repositories"))
