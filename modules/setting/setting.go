...
import (
	...
	"github.com/kumose/kmup/modules/user"
)

// settings
var (
	// AppVer is the version of the current build of Kmup. It is set in main.go from main.Version.
	AppVer string
	// AppBuiltWith represents a human-readable version go runtime build version and build tags. (See main.go formatBuiltWith().)
	AppBuiltWith string
	// AppStartTime store time kmup has started
...
func PrepareAppDataPath() error {
	...
	// For example, if someDir=/mnt/vol1/kmup-home/data, if the mount point /mnt/vol1 is not mounted when Kmup runs,
	// then kmup will make new empty directories in /mnt/vol1, all are stored in the root filesystem.
...
func MustInstalled() {
	if !InstallLock {
		log.Fatal(`Unable to load config file for a installed Kmup instance, you should either use "--config" to set your config file (app.ini), or run "kmup web" command to install Kmup.`)
	}
}
...
func loadRunModeFrom(rootCfg ConfigProvider) {
	...
	// The following is a purposefully undocumented option. Please do not run Kmup as root. It will only cause future headaches.
	// Please don't use root as a bandaid to "fix" something that is broken, instead the broken thing should instead be fixed properly.
	unsafeAllowRunAsRoot := ConfigSectionKeyBool(rootSec, "I_AM_BEING_UNSAFE_RUNNING_AS_ROOT")
	unsafeAllowRunAsRoot = unsafeAllowRunAsRoot || optional.ParseBool(os.Getenv("KMUP_I_AM_BEING_UNSAFE_RUNNING_AS_ROOT")).Value()
	RunMode = os.Getenv("KMUP_RUN_MODE")
	if RunMode == "" {
	...
	if os.Getuid() == 0 {
		if !unsafeAllowRunAsRoot {
			...
			log.Fatal("Kmup is not supposed to be run as root. Sorry. If you need to use privileged TCP ports please instead use setcap and the `cap_net_bind_service` permission")
		}
		log.Critical("You are running Kmup using the root user, and have purposely chosen to skip built-in protections around this. You have been warned against this.")
	}
}
