...
type testSerializationStruct struct {
	...
	// The behavior is inconsistent between json and v2 packages, and there is no such use case in Kmup.
...
func TestOptionalToJson(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, tc.want, string(b), "kmup json module returned unexpected")
...
func TestOptionalFromJson(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			assert.Equal(t, tc.want, obj1, "kmup json module returned unexpected")
