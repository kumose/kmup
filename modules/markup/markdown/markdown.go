...
import (
	...
	"github.com/kumose/kmup/modules/markup/markdown/math"
...
func (r *GlodmarkRender) highlightingRenderer(w util.BufWriter, c highlighting.CodeBlockContext, entering bool) {
	if entering {
		...
		languageStr := kmuputil.IfZero(string(languageBytes), "text")
...
// SpecializedMarkdown sets up the Kmup specific markdown extensions
...
func render(ctx *markup.RenderContext, input io.Reader, output io.Writer) error {
	...
	defer func() {
		...
		escapedHTML := template.HTMLEscapeString(kmuputil.UnsafeBytesToString(buf))
		_, _ = output.Write(kmuputil.UnsafeStringToBytes(escapedHTML))
	}()
