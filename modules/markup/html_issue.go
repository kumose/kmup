...
import (
	...
	"github.com/kumose/kmup/modules/regexplru"
	"github.com/kumose/kmup/modules/templates/vars"
...
func fullIssuePatternProcessor(ctx *RenderContext, node *html.Node) {
	...
	for node != nil && node != next {
		...
		if !httplib.IsCurrentKmupSiteURL(ctx, link) {
			return
		}
		...
		// http://localhost:3326/gituser/myrepo/issues/1
...
func issueIndexPatternProcessor(ctx *RenderContext, node *html.Node) {
	...
	for node != nil && node != next {
		...
		if hasExtTrackFormat && !ref.IsPull {
			...
			// Kmup will redirect on click as appropriate.
