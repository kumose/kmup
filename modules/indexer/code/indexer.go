...
import (
	...
	"github.com/kumose/kmup/modules/indexer/code/bleve"
	"github.com/kumose/kmup/modules/indexer/code/elasticsearch"
	"github.com/kumose/kmup/modules/indexer/code/internal"
		...
		case "bleve":
			...
			defer func() {
				if err := recover(); err != nil {
					...
					log.Error("You can completely remove the \"%s\" directory to make Kmup recreate the indexes", setting.Indexer.RepoPath)
				}
			}()
		...
		case "elasticsearch":
			...
			defer func() {
				if err := recover(); err != nil {
					log.Error("PANIC whilst initializing repository indexer: %v\nStacktrace: %s", err, log.Stack(2))
					log.Error("The indexer files are likely corrupted and may need to be deleted")
					log.Error("You can completely remove the \"%s\" index to make Kmup recreate the indexes", util.SanitizeCredentialURLs(setting.Indexer.RepoConnStr))
				}
			}()
