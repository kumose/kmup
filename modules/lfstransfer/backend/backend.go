...
var _ transfer.Backend = (*KmupBackend)(nil)

// KmupBackend is an adapter between git-lfs-transfer library and Kmup's internal LFS API
type KmupBackend struct {
	ctx          context.Context
	server       *url.URL
	op           string
	authToken    string
...
func New(ctx context.Context, repo, op, token string, logger transfer.Logger) (transfer.Backend, error) {
	...
	return &KmupBackend{ctx: ctx, server: server, op: op, authToken: token, internalAuth: "Bearer " + setting.InternalToken, logger: logger}, nil
}

// Batch implements transfer.Backend
func (g *KmupBackend) Batch(_ string, pointers []transfer.BatchItem, args transfer.Args) ([]transfer.BatchItem, error) {
...
func (g *KmupBackend) Download(oid string, args transfer.Args) (_ io.ReadCloser, _ int64, retErr error) {
	...
	respSize, err := strconv.ParseInt(resp.Header.Get("X-Kmup-LFS-Content-Length"), 10, 64)
	if err != nil {
...
func (g *KmupBackend) Upload(oid string, size int64, r io.Reader, args transfer.Args) error {
	idMapStr, exists := args[argID]
	if !exists {
		return ErrMissingID
	}
...
func (g *KmupBackend) Verify(oid string, size int64, args transfer.Args) (transfer.Status, error) {
...
func (g *KmupBackend) LockBackend(_ transfer.Args) transfer.LockBackend {
	return newKmupLockBackend(g)
}
