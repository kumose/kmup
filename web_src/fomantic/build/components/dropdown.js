...
$.fn.dropdown = function(parameters) {
  var
    ...
    // KMUP-PATCH: always "click" as clickEvent, old code used "touchstart" as clickEvent, it is wrong,
  ...
  $allModules
    .each(function(elementIndex) {
      var
        ...
        $module         = $(this),
        $context        = (typeof settings.context === 'string') ? $(document).find(settings.context) : $(settings.context), // KMUP-PATCH: use "jQuery.find(selector)" instead of "jQuery(selector)"
      ...
      module = {
        ...
        show: function(callback, preventFocus) {
          ...
          if( module.can.show() && !module.is.active() ) {
            ...
            if(settings.onShow.call(element) !== false) {
              $module.fomanticExt.onDropdownAfterFiltered.call(element); // KMUP-PATCH: callback to correctly handle the filtered items
            ...
            if( module.can.useAPI() ) {
              module.queryRemote(searchTerm, function() {
                ...
                $.each(preSelected,function(index,value){
                  $item.filter('[data-value="'+CSS.escape(value)+'"]') // KMUP-PATCH: use "CSS.escape" for query selector
        ...
        change: {
          values: function(values) {
            ...
            if(module.has.selectInput()) {
              ...
              $.each(values, function(index, item) {
                var
                  value = settings.templates.escape(item[fields.value]), // KMUP-PATCH: use "escape" for attribute value
        ...
        event: {
          ...
          icon: {
            click: function(event) {
              iconClicked=true;
              // KMUP-PATCH: official dropdown doesn't support the search input in menu
...
$.fn.dropdown.settings = {
  ...
  selector : {
    ...
    sizer        : '> span.sizer',
    text         : '> .text:not(.icon)',
    unselectable : '.disabled, .filtered, .tw-hidden', // KMUP-PATCH: tw-hidden hides the item so it is also unselectable
      ...
      if( itemType === 'item' ) {
        var
          maybeText = (option[fields.text])
            ? ' data-text="' + escape(option[fields.text]) + '"' // KMUP-PATCH: use "escape" for attribute value
            : '',
          maybeDisabled = (option[fields.disabled])
            ? className.disabled+' '
            : ''
        ;
        // KMUP-PATCH: use "escape" for attribute value
