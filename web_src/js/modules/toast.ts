...
type ToastifyElement = HTMLElement & {_kmupToastifyInstance?: Toast};
...
function showToast(message: string, level: Intent, {gravity, position, duration, useHtmlBody, preventDuplicates = true, ...other}: ToastOpts = {}): Toast | null {
  ...
  (toast.toastElement as ToastifyElement)._kmupToastifyInstance = toast;
  return toast;
}
...
function hideToastByElement(el: Element): void {
  (el as ToastifyElement)?._kmupToastifyInstance?.hideToast();
}
