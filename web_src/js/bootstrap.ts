...
function shouldIgnoreError(err: Error) {
  const ignorePatterns = [
    '/assets/js/monaco.', // https://github.com/go-kmup/kmup/issues/30861 , https://github.com/microsoft/monaco-editor/issues/4496
  ];
...
function processWindowErrorEvent({error, reason, message, type, filename, lineno, colno}: ErrorEvent & PromiseRejectionEvent) {
  ...
  // - https://github.com/go-kmup/kmup/issues/20240
  if (!err) {
...
function initGlobalErrorHandler() {
  ...
  if (!window.config) {
    showGlobalErrorMessage(`Kmup JavaScript code couldn't run correctly, please check your custom templates`);
  }
