...
RENDER_COMMAND = `echo '<div style="width: 100%; height: 2000px; border: 10px solid red; box-sizing: border-box;"><a href="/">a link</a> <a target="_blank" href="//kmup.com">external link</a></div>'`
...
function mainExternalRenderIframe() {
  ...
  const iframeId = u.searchParams.get('kmup-iframe-id');
  ...
  const postIframeMsg = (cmd: string, data: Record<string, any> = {}) => {
    window.parent.postMessage({kmupIframeCmd: cmd, kmupIframeId: iframeId, ...data}, '*');
  };
