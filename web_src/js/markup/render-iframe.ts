...
export async function loadRenderIframeContent(iframe: HTMLIFrameElement) {
  ...
  if (!iframe.id) iframe.id = generateElemId('kmup-iframe-');

  window.addEventListener('message', (e) => {
    if (!e.data?.kmupIframeCmd || e.data?.kmupIframeId !== iframe.id) return;
    const cmd = e.data.kmupIframeCmd;
    if (cmd === 'resize') {
      ...
      throw new Error(`Unknown kmup iframe cmd: ${cmd}`);
    }
  });

  const u = new URL(iframeSrcUrl, window.location.origin);
  u.searchParams.set('kmup-is-dark-theme', String(isDarkTheme()));
  u.searchParams.set('kmup-iframe-id', iframe.id);
  iframe.src = u.href;
}
