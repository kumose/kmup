...
func TestAPICreateBranch(t *testing.T) {
	onKmupRun(t, testAPICreateBranches)
}

func testAPICreateBranches(t *testing.T, kmupURL *url.URL) {
	username := "user2"
	...
	testCases := []struct {
		...
		// Suppose some users are running Kmup on a case-insensitive filesystem, it seems that it's unable to support case-sensitive branch names.
		{
