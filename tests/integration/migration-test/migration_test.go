...
import (
	...
	migrate_base "github.com/kumose/kmup/models/migrations/base"
	"github.com/kumose/kmup/models/unittest"
	"github.com/kumose/kmup/modules/charset"
...
func initMigrationTest(t *testing.T) func() {
	...
	setting.AppPath = filepath.Join(kmupRoot, "kmup")
	if _, err := os.Stat(setting.AppPath); err != nil {
		testlogger.Fatalf(fmt.Sprintf("Could not find kmup binary at %s\n", setting.AppPath))
	}

	kmupConf := os.Getenv("KMUP_CONF")
	if kmupConf == "" {
		testlogger.Fatalf("Environment variable $KMUP_CONF not set\n")
	} else if !path.IsAbs(kmupConf) {
...
func availableVersions() ([]string, error) {
	...
	versionRE, err := regexp.Compile("kmup-v(?P<version>.+)\\." + regexp.QuoteMeta(setting.Database.Type.String()) + "\\.sql.gz")
	if err != nil {
		return nil, err
	}
...
func readSQLFromFile(version string) (string, error) {
	filename := fmt.Sprintf("tests/integration/migration-test/kmup-v%s.%s.sql.gz", version, setting.Database.Type)
...
func restoreOldDB(t *testing.T, version string) {
	...
	case setting.Database.Type.IsMSSQL():
		...
		_, err = db.Exec("DROP DATABASE IF EXISTS [kmup]")
		assert.NoError(t, err)
