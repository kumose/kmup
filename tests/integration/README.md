...
Start tests based on the database container
```
TEST_MSSQL_HOST=localhost:1433 TEST_MSSQL_DBNAME=kmup_test TEST_MSSQL_USERNAME=sa TEST_MSSQL_PASSWORD=MwantsaSecurePassword1 make test-mssql
```
...
```bash
KMUP_TEST_SLOW_RUN="10s" KMUP_TEST_SLOW_FLUSH="1s" make test-sqlite
```
