	...
	for _, image := range images {
		t.Run(fmt.Sprintf("[Image:%s]", image), func(t *testing.T) {
			...
			t.Run("UploadBlob/Mount", func(t *testing.T) {
				...
				req := NewRequestWithBody(t, "POST", fmt.Sprintf("%sv2/%s/%s/blobs/uploads?digest=%s", setting.AppURL, privateUser.Name, image, privateBlobDigest), strings.NewReader("kmup")).
			...
			t.Run("GetTagList", func(t *testing.T) {
				...
				cases := []struct {
					...
					{
						URL:          url + "/tags/list",
						ExpectedTags: []string{"latest", "master", "multi"},
					...
					},
					{
						...
						ExpectedTags: []string{"latest", "master"},
						ExpectedLink: fmt.Sprintf(`</v2/%s/%s/tags/list?last=master&n=2>; rel="next"`, user.Name, image),
					},
					{
						URL:          url + "/tags/list?last=master",
						ExpectedTags: []string{"multi"},
						ExpectedLink: fmt.Sprintf(`</v2/%s/%s/tags/list?last=multi>; rel="next"`, user.Name, image),
					},
					{
						URL:          url + "/tags/list?n=1&last=latest",
						ExpectedTags: []string{"master"},
						ExpectedLink: fmt.Sprintf(`</v2/%s/%s/tags/list?last=master&n=1>; rel="next"`, user.Name, image),
					},
				}
				...
				assert.Len(t, apiPackages, 4) // "latest", "master", "multi", "sha256:..."
			})
