...
func TestPackageMaven(t *testing.T) {
	...
	root := "/api/packages/user2/maven/com/kmup/test-project"
	...
	t.Run("UploadLegacy", func(t *testing.T) {
		...
		legacyRootLink := "/api/packages/user2/maven/com/kmup/legacy-project"
		...
		p, err := packages.GetPackageByName(t.Context(), user.ID, packages.TypeMaven, "com.kmup-legacy-project")
		require.NoError(t, err)
		assert.Equal(t, "com.kmup-legacy-project", p.Name)

		req = NewRequest(t, "HEAD", legacyRootLink+"/1.0.2/any-file-name").AddBasicAuth(user.Name)
		MakeRequest(t, req, http.StatusOK)

		req = NewRequest(t, "GET", "/user2/-/packages/maven/com.kmup-legacy-project/1.0.2")
		MakeRequest(t, req, http.StatusOK)
		req = NewRequest(t, "GET", "/user2/-/packages/maven/com.kmup:legacy-project/1.0.2")
		MakeRequest(t, req, http.StatusNotFound)
		req = NewRequest(t, "GET", "/user2/-/packages/maven/com.kmup%3Alegacy-project/1.0.2")
		MakeRequest(t, req, http.StatusNotFound)
		...
		MakeRequest(t, req, http.StatusCreated)
		_, err = packages.GetPackageByName(t.Context(), user.ID, packages.TypeMaven, "com.kmup-legacy-project")
...
<project xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
  ...
  <description>` + packageDescription + `</description>
	...
	t.Run("DownloadMetadata", func(t *testing.T) {
		...
		expectedMetadata := `<?xml version="1.0" encoding="UTF-8"?>` + "\n<metadata><groupId>com.kmup</groupId><artifactId>test-project</artifactId><versioning><release>1.0.1</release><latest>1.0.1</latest><versions><version>1.0.1</version></versions></versioning></metadata>"
