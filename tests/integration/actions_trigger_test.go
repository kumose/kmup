...
import (
	...
	actions_module "github.com/kumose/kmup/modules/actions"
	...
	commitstatus_service "github.com/kumose/kmup/services/repository/commitstatus"
	...
	onKmupRun(t, func(t *testing.T, u *url.URL) {
		...
		addWorkflowToBaseResp, err := files_service.ChangeRepoFiles(t.Context(), baseRepo, user2, &files_service.ChangeRepoFilesOptions{
			Files: []*files_service.ChangeRepoFile{
				{
					Operation: "create",
					TreePath:  ".kmup/workflows/pr.yml",
	...
	onKmupRun(t, func(t *testing.T, u *url.URL) {
		...
		addWorkflowToBaseResp, err := files_service.ChangeRepoFiles(t.Context(), repo, user2, &files_service.ChangeRepoFilesOptions{
			Files: []*files_service.ChangeRepoFile{
				{
					Operation: "create",
					TreePath:  ".kmup/workflows/createdelete.yml",
					ContentReader: strings.NewReader(`name: test
on:
  [create,delete]
jobs:
...
`),
		...
		err = repo_service.CreateNewBranch(t.Context(), user2, repo, "master", testBranch)
		assert.NoError(t, err)
	...
	onKmupRun(t, func(t *testing.T, u *url.URL) {
		...
		addWorkflowToBaseResp, err := files_service.ChangeRepoFiles(t.Context(), repo, user2, &files_service.ChangeRepoFilesOptions{
			Files: []*files_service.ChangeRepoFile{
				{
					Operation: "create",
					TreePath:  ".kmup/workflows/dispatch.yml",
...
`),
		...
		assert.Equal(t, ".kmup/workflows/dispatch.yml", workflows.Workflows[0].Path)
		assert.Equal(t, ".kmup/workflows/dispatch.yml", workflows.Workflows[0].Path)
...
`
		...
		doAPICreateFile(user4APICtx, "app/main.go", &api.CreateFileOptions{
			FileOptions: api.FileOptions{
				NewBranchName: "user4/add-master",
		...
		apiPull, err := doAPICreatePullRequest(user4APICtx, baseRepo.OwnerName, baseRepo.Name, baseRepo.DefaultBranch, user4.Name+":user4/add-master")(t)
		assert.NoError(t, err)
	...
	onKmupRun(t, func(t *testing.T, u *url.URL) {
		...
		addWorkflowToBaseResp, err := files_service.ChangeRepoFiles(t.Context(), repo, user2, &files_service.ChangeRepoFilesOptions{
			Files: []*files_service.ChangeRepoFile{
				{
					Operation: "create",
					TreePath:  ".kmup/workflows/runname.yml",
					ContentReader: strings.NewReader(`name: test
on:
  [create,delete]
run-name: ${{ kmup.actor }} is running this workflow
jobs:
  test:
    runs-on: ubuntu-latest
    steps:
