...
import (
	...
	debian_module "github.com/kumose/kmup/modules/packages/debian"
	packages_cleanup_service "github.com/kumose/kmup/services/packages/cleanup"
...
func TestPackageDebian(t *testing.T) {
	...
	distributions := []string{"test", "kmup"}
	components := []string{"master", "stable"}
	...
	t.Run("Cleanup", func(t *testing.T) {
		...
		for i := range packagesCount {
			uploadURL := fmt.Sprintf("%s/pool/%s/%s/upload", rootURL, "test", "master")
