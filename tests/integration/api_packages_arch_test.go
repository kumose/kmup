...
import (
	...
	arch_service "github.com/kumose/kmup/services/packages/arch"
...
license = MIT`)
	...
	repositories := []string{"master", "testing", "with/slash", ""}
	...
	t.Run("KeepLastVersion", func(t *testing.T) {
		defer tests.PrintCurrentTest(t)()
		pkgVer1 := createPackage("gz", "kmup-test", "1.0.0", "aarch64")
		pkgVer2 := createPackage("gz", "kmup-test", "1.0.1", "aarch64")
		...
		_, has := content["kmup-test-1.0.0/desc"]
		assert.False(t, has)
		_, has = content["kmup-test-1.0.1/desc"]
		assert.True(t, has)

		req = NewRequest(t, "DELETE", rootURL+"/kmup-test/1.0.1/aarch64").
			AddBasicAuth(user.Name)
		MakeRequest(t, req, http.StatusNoContent)
