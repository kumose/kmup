...
func Test_WebhookPackage(t *testing.T) {
	onKmupRun(t, func(t *testing.T, kmupURL *url.URL) {
		...
		// 2. trigger the webhook
		token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeAll)
		url := fmt.Sprintf("/api/packages/%s/generic/%s/%s", "org3", "kmup", "v1.24.0")
		req := NewRequestWithBody(t, "PUT", url+"/kmup", strings.NewReader("This is a dummy file")).
			AddTokenAuth(token)
		MakeRequest(t, req, http.StatusCreated)
		...
		assert.Len(t, payloads, 1)
		assert.EqualValues(t, "created", payloads[0].Action)
		assert.Equal(t, "kmup", payloads[0].Package.Name)
...
func Test_WebhookStatus(t *testing.T) {
	onKmupRun(t, func(t *testing.T, kmupURL *url.URL) {
		...
		provider := newMockWebhookProvider(func(r *http.Request) {
			...
			assert.Contains(t, r.Header["X-Kmup-Event-Type"], "status", "X-Kmup-Event-Type should contain status")
			assert.Contains(t, r.Header["X-Kmup-Hook-Installation-Target-Type"], "repository", "X-Kmup-Hook-Installation-Target-Type should contain repository")
...
func Test_WebhookStatus_NoWrongTrigger(t *testing.T) {
	onKmupRun(t, func(t *testing.T, kmupURL *url.URL) {
		...
		provider := newMockWebhookProvider(func(r *http.Request) {
			...
			assert.NotContains(t, r.Header["X-Kmup-Event-Type"], "status", "X-Kmup-Event-Type should not contain status")
		...
		testCreateWebhookForRepo(t, session, "kmup", "user2", "repo1", provider.URL(), "push_only")
...
func Test_WebhookWorkflowJob(t *testing.T) {
	onKmupRun(t, func(t *testing.T, kmupURL *url.URL) {
		...
		provider := newMockWebhookProvider(func(r *http.Request) {
			...
			assert.Contains(t, r.Header["X-Kmup-Event-Type"], "workflow_job", "X-Kmup-Event-Type should contain workflow_job")
		...
		// init the workflow
		wfTreePath := ".kmup/workflows/push.yml"
		...
		t.Run(obj.name, func(t *testing.T) {
			onKmupRun(t, func(t *testing.T, kmupURL *url.URL) {
				...
				provider := newMockWebhookProvider(func(r *http.Request) {
					...
					assert.Contains(t, r.Header["X-Kmup-Event-Type"], "workflow_run", "X-Kmup-Event-Type should contain workflow_run")
...
func testWorkflowRunEventsOnCancellingAbandonedRun(t *testing.T, webhookData *workflowRunWebhook, allJobsAbandoned bool) {
	...
	// init the workflow
	wfilename := "push.yml"
	wfTreePath := ".kmup/workflows/" + wfilename
	wfFileContent := `on:
  push:
  workflow_dispatch:
...
jobs:
  dispatch:
    ...
    steps:
      - run: echo 'test the webhook'
`)
	createWorkflowFile(t, token, "user2", "repo1", ".kmup/workflows/dispatch.yml", opts)
