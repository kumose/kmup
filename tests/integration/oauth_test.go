...
func testOAuth2WellKnown(t *testing.T) {
	defer test.MockVariableValue(&setting.AppURL, "https://try.kmup.io/")()
	...
	t.Run("WellKnown", func(t *testing.T) {
		...
		assert.Equal(t, "https://try.kmup.io", respMap["issuer"])
		assert.Equal(t, "https://try.kmup.io/login/oauth/authorize", respMap["authorization_endpoint"])
		assert.Equal(t, "https://try.kmup.io/login/oauth/access_token", respMap["token_endpoint"])
		assert.Equal(t, "https://try.kmup.io/login/oauth/keys", respMap["jwks_uri"])
		assert.Equal(t, "https://try.kmup.io/login/oauth/userinfo", respMap["userinfo_endpoint"])
		assert.Equal(t, "https://try.kmup.io/login/oauth/introspect", respMap["introspection_endpoint"])
		assert.Equal(t, []any{"RS256"}, respMap["id_token_signing_alg_values_supported"])
	})

	t.Run("WellKnownWithIssuer", func(t *testing.T) {
		defer test.MockVariableValue(&setting.OAuth2.JWTClaimIssuer, "https://try.kmup.io/")()
		req := NewRequest(t, "GET", urlOpenidConfiguration)
		resp := MakeRequest(t, req, http.StatusOK)
		var respMap map[string]any
		DecodeJSON(t, resp, &respMap)
		assert.Equal(t, "https://try.kmup.io/", respMap["issuer"]) // has trailing by JWTClaimIssuer
		assert.Equal(t, "https://try.kmup.io/login/oauth/authorize", respMap["authorization_endpoint"])
	})
