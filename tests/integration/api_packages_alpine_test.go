...
AACAX/AKARNTyAAoAAA=`
	...
	repositories := []string{"master", "testing"}
		...
		for _, repository := range repositories {
			t.Run(fmt.Sprintf("[Branch:%s,Repository:%s]", branch, repository), func(t *testing.T) {
				...
				t.Run("Index", func(t *testing.T) {
					...
					assert.Contains(t, content, "T:Kmup Test Package\n")
					assert.Contains(t, content, "U:https://kmup.io/\n")
					assert.Contains(t, content, "L:MIT\n")
					assert.Contains(t, content, "S:1353\n")
					assert.Contains(t, content, "I:4096\n")
					assert.Contains(t, content, "o:kmup-test\n")
					assert.Contains(t, content, "m:KN4CK3R <kn4ck3r@kmup.io>\n")
				...
				t.Run("NoArch", func(t *testing.T) {
					...
					for _, arch := range []string{alpine_module.NoArch, "x86_64", "my_arch"} {
						req := NewRequest(t, "GET", fmt.Sprintf("%s/%s/%s/%s/kmup-noarch-1.4-r0.apk", rootURL, branch, repository, arch))
						MakeRequest(t, req, http.StatusOK)
					}

					req = NewRequest(t, "DELETE", fmt.Sprintf("%s/%s/%s/noarch/kmup-noarch-1.4-r0.apk", rootURL, branch, repository)).
						AddBasicAuth(user.Name)
