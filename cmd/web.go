...
import (
	...
	"github.com/kumose/kmup/routers/install"
...
var PIDFile = "/run/kmup.pid"

// CmdWeb represents the available web sub-command.
var CmdWeb = &cli.Command{
	Name:  "web",
	Usage: "Start Kmup web server",
	Description: `Kmup web server is the only thing you need to run,
...
func showWebStartupMessage(msg string) {
	log.Info("Kmup version: %s%s", setting.AppVer, setting.AppBuiltWith)
...
func serveInstall(cmd *cli.Command) error {
	...
	err := listen(c, false)
	if err != nil {
		log.Critical("Unable to open listener for installer. Is Kmup already running?")
		graceful.GetManager().DoGracefulShutdown()
	}
	select {
	case <-graceful.GetManager().IsShutdown():
		<-graceful.GetManager().Done()
		log.Info("PID: %d Kmup Web Finished", os.Getpid())
		return err
	default:
	}
	return nil
...
func serveInstalled(c *cli.Command) error {
	...
	// now, Kmup only serves pre-defined files in the "custom/public" folder basing on the web root, the user should move their custom files to "custom/public/assets"
...
func runWeb(ctx context.Context, cmd *cli.Command) error {
	...
	if os.Getppid() > 1 && len(os.Getenv("LISTEN_FDS")) > 0 {
		log.Info("Restarting Kmup on PID: %d from parent PID: %d", os.Getpid(), os.Getppid())
	} else {
		log.Info("Starting Kmup on PID: %d", os.Getpid())
	}
...
func listen(m http.Handler, handleRedirector bool) error {
	...
	_, _, finished := process.GetManager().AddTypedContext(graceful.GetManager().HammerContext(), "Web: Kmup Server", process.SystemProcessType, true)
	defer finished()
