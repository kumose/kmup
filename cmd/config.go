...
func cmdConfig() *cli.Command {
	subcmdConfigEditIni := &cli.Command{
		...
		Description: `
Help users to edit the Kmup configuration INI file.
...
$ ./kmup config edit-ini --config app-old.ini --config-keep-keys app-keys.ini --out app-new.ini

# Map Environment Variables to INI Configuration

Environment variables of the form "KMUP__section_name__KEY_NAME"
will be mapped to the ini section "[section_name]" and the key
"KEY_NAME" with the value as provided.

Environment variables of the form "KMUP__section_name__KEY_NAME__FILE"
will be mapped to the ini section "[section_name]" and the key
...
$ export KMUP__git_0x2E_config__foo_0x2E_bar=val

# Put All Together

$ ./kmup config edit-ini --config app.ini --config-keep-keys app-keys.ini --apply-env {--in-place|--out app-new.ini}
`,
		Flags: []cli.Flag{
			...
			&cli.BoolFlag{
				Name:  "apply-env",
				Usage: "Apply all KMUP__* variables from the environment to the config.",
			},
			&cli.StringFlag{
				Name:  "out",
	...
	return &cli.Command{
		Name:  "config",
		Usage: "Manage Kmup configuration",
