...
import (
	...
	"github.com/kumose/kmup/modules/dump"
...
var CmdDump = &cli.Command{
	Name:        "dump",
	Usage:       "Dump Kmup files and database",
	Description: `Dump compresses all related files and database into zip file. It can be used for backup and capture Kmup server image to send to maintainer`,
	Action:      runDump,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:    "file",
			Aliases: []string{"f"},
			Usage:   `Name of the dump file which will be created, default to "kmup-dump-{time}.zip". Supply '-' for stdout. See type for available types.`,
		},
		&cli.BoolFlag{
			Name:    "verbose",
			Aliases: []string{"V"},
...
func runDump(ctx context.Context, cmd *cli.Command) error {
	...
	if cmd.Bool("skip-db") {
		...
		dbDump, err := os.CreateTemp(tmpDir, "kmup-db.sql")
		if err != nil {
		...
		if err = dumper.AddFileByPath("kmup-db.sql", dbDump.Name()); err != nil {
			fatal("Failed to include kmup-db.sql: %v", err)
		}
	}
