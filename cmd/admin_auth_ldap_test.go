...
import (
	...
	"github.com/kumose/kmup/services/auth/source/ldap"
	...
	cases := []struct {
		...
		// case 0
		{
			args: []string{
				...
				"--group-filter", "(|(cn=kmup_users)(cn=admins))",
				"--group-team-map", `{"cn=my-group,cn=groups,dc=example,dc=org": {"MyKmupOrganization": ["MyKmupTeam1", "MyKmupTeam2"]}}`,
			...
			source: &auth.Source{
				...
				Cfg: &ldap.Source{
					...
					UserUID:               "uid",
					GroupFilter:           "(|(cn=kmup_users)(cn=admins))",
					GroupTeamMap:          `{"cn=my-group,cn=groups,dc=example,dc=org": {"MyKmupOrganization": ["MyKmupTeam1", "MyKmupTeam2"]}}`,
