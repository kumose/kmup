...
import (
	...
	"github.com/kumose/kmup/tools/codeformat"
)

// Windows has a limitation for command line arguments, the size can not exceed 32KB.
// So we have to feed the files to some tools (like gofmt) batch by batch

// We also introduce a `kmup-fmt` command, it does better import formatting than gofmt/goimports. `kmup-fmt` calls `gofmt` internally.
...
func newFileCollector(fileFilter string, batchSize int) (*fileCollector, error) {
	...
	if fileFilter == "go-own" {
		...
		co.excludePatterns = append(co.excludePatterns, regexp.MustCompile(`tests/kmup-repositories-meta`))
...
func kmupFormatGoImports(files []string, doWriteFile bool) error {
	...
	for _, files := range fileBatches {
		...
		case "kmup-fmt":
			if slices.Contains(subArgs, "-d") {
				log.Print("the -d option is not supported by kmup-fmt")
			}
			cmdErrors = append(cmdErrors, kmupFormatGoImports(files, slices.Contains(subArgs, "-w")))
