...
func HookPostReceive(ctx *kmup_context.PrivateContext) {
...
func handlePullRequestMerging(ctx *kmup_context.PrivateContext, opts *private.HookOptions, ownerName, repoName string, updates []*repo_module.PushUpdateOptions) {
	if len(updates) == 0 {
		ctx.JSON(http.StatusInternalServerError, private.HookPostReceiveResult{
