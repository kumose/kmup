...
import (
	...
	db_install "github.com/kumose/kmup/models/db/install"
...
func checkDatabase(ctx *context.Context, form *forms.InstallForm) bool {
	...
	if err = db.InitEngine(ctx); err != nil {
		if strings.Contains(err.Error(), `Unknown database type: sqlite3`) {
			...
			ctx.RenderWithErr(ctx.Tr("install.sqlite3_not_available", "https://docs.kmup.com/installation/install-from-binary"), tplInstall, form)
		} else {
	...
	if hasPostInstallationUser && dbMigrationVersion > 0 {
		log.Error("The database is likely to have been used by Kmup before, database migration version=%d", dbMigrationVersion)
		...
		log.Info("User confirmed re-installation of Kmup into a pre-existing database")
	}

	if hasPostInstallationUser || dbMigrationVersion > 0 {
		log.Info("Kmup will be installed in a database with: hasPostInstallationUser=%v, dbMigrationVersion=%v", hasPostInstallationUser, dbMigrationVersion)
	}
...
func SubmitInstall(ctx *context.Context) {
	...
	// Prepare AppDataPath, it is very important for Kmup
	...
	go func() {
		...
		// What if this duration is not long enough? That's impossible -- if the user can't load the simple page in time, how could they install or use Kmup in the future ....
