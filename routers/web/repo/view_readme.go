...
func findReadmeFileInEntries(ctx *context.Context, parentDir string, entries []*git.TreeEntry, tryWellKnownDirs bool) (string, *git.TreeEntry, error) {
	docsEntries := make([]*git.TreeEntry, 3) // (one of docs/, .kmup/ or .github/)
	for _, entry := range entries {
		if tryWellKnownDirs && entry.IsDir() {
			// as a special case for the top-level repo introduction README,
			// fall back to subfolders, looking for e.g. docs/README.md, .kmup/README.zh-CN.txt, .github/README.txt, ...
			...
			case ".kmup":
				if entry.Name() == ".kmup" || docsEntries[1] == nil {
					docsEntries[1] = entry
				}
			case ".github":
