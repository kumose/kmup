...
func checkOutdatedBranch(ctx *context.Context) {
	...
	if dbBranch.CommitID != commit.ID.String() {
		ctx.Flash.Warning(ctx.Tr("repo.error.broken_git_hook", "https://docs.kmup.com/help/faq#push-hook--webhook--actions-arent-running"), true)
	}
}
...
func prepareOpenWithEditorApps(ctx *context.Context) {
	...
	for _, app := range apps {
		...
		case "vscodium":
			iconName = "kmup-vscodium"
		case "jetbrains":
			iconName = "kmup-jetbrains"
		default:
...
func handleRepoViewSubmodule(ctx *context.Context, commitSubmoduleFile *git.CommitSubmoduleFile) {
	...
	} else if !httplib.IsCurrentKmupSiteURL(ctx, redirectLink) {
...
func RedirectRepoTreeToSrc(ctx *context.Context) {
	...
	// This is done intentionally so that Kmup's repo URL structure matches other forges (GitHub/GitLab) provide,
	...
	// * "https://kmup/owner/repo/tree/{CommitID}"
