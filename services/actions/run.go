...
func PrepareRunAndInsert(ctx context.Context, content []byte, run *actions_model.ActionRun, inputsWithDefaults map[string]any) error {
	...
	kmupCtx := GenerateKmupContext(run, nil)

	jobs, err := jobparser.Parse(content, jobparser.WithVars(vars), jobparser.WithGitContext(kmupCtx.ToGitHubContext()), jobparser.WithInputs(inputsWithDefaults))
	if err != nil {
