...
type KmupContext map[string]any

// GenerateKmupContext generate the kmup context without token and kmup_runtime_token
// job can be nil when generating a context for parsing workflow-level expressions
func GenerateKmupContext(run *actions_model.ActionRun, job *actions_model.ActionRunJob) KmupContext {
	...
	gitContext := KmupContext{
		...
		// additional contexts
		"kmup_default_actions_url": setting.Actions.DefaultActionsURL.URL(),
	}
...
func (g *KmupContext) ToGitHubContext() *model.GithubContext {
	return &model.GithubContext{
		...
		JobName:          "", // not present in KmupContext
		RepositoryOwner:  util.GetMapValueOrDefault(*g, "repository_owner", ""),
		RetentionDays:    util.GetMapValueOrDefault(*g, "retention_days", ""),
		RunnerPerflog:    "", // not present in KmupContext
		RunnerTrackingID: "", // not present in KmupContext
