...
func SendTestMail(email string) error {
	...
	return sender_service.Send(sender, sender_service.NewMessage(email, "Kmup Test Email!", "Kmup Test Email!"))
}
			...
			if n.Data == "img" {
				for i, attr := range n.Attr {
					if attr.Key == "src" {
						...
						if err != nil {
							// Not an error, just skip. This is probably an image from outside the kmup instance.
...
func (b64embedder *mailAttachmentBase64Embedder) AttachmentSrcToBase64DataURI(ctx context.Context, attachmentSrc string) (string, error) {
	parsedSrc := httplib.ParseKmupSiteURL(ctx, attachmentSrc)
...
func generateMetadataHeaders(repo *repo_model.Repository) map[string]string {
	return map[string]string{
		...
		"X-Kmup-Repository":      repo.Name,
		"X-Kmup-Repository-Path": repo.FullName(),
		"X-Kmup-Repository-Link": repo.HTMLURL(),
...
func generateSenderRecipientHeaders(doer, recipient *user_model.User) map[string]string {
	return map[string]string{
		"X-Kmup-Sender":              doer.Name,
		"X-Kmup-Recipient":           recipient.Name,
		"X-Kmup-Recipient-Address":   recipient.Email,
