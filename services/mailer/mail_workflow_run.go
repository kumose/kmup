...
func MailActionsTrigger(ctx context.Context, sender *user_model.User, repo *repo_model.Repository, run *actions_model.ActionRun) error {
	...
	if !sender.IsKmupActions() && !sender.IsGhost() && sender.IsMailable() {
		notifyPref, err := user_model.GetUserSetting(ctx, sender.ID,
			user_model.SettingsKeyEmailNotificationKmupActions, user_model.SettingEmailNotificationKmupActionsFailureOnly)
		if err != nil {
			return err
		}
		if notifyPref == user_model.SettingEmailNotificationKmupActionsAll || !run.Status.IsSuccess() && notifyPref != user_model.SettingEmailNotificationKmupActionsDisabled {
