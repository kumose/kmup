...
import (
	...
	"github.com/kumose/kmup/services/auth/source/sspi"
...
func (s *SSPI) Verify(req *http.Request, w http.ResponseWriter, store DataStore, sess SessionStore) (*user_model.User, error) {
	...
	if err != nil {
		...
		// in this case, the Verify function is called in Kmup's web context
		// FIXME: it doesn't look good to render the page here, why not redirect?
		kmup_context.GetWebContext(req.Context()).HTML(http.StatusUnauthorized, tplSignIn)
		return nil, err
	}
	if outToken != "" {
		sspiAuth.AppendAuthenticateHeader(w, outToken)
