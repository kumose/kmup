...
func addDefaultHeaders(req *http.Request, secret []byte, w *webhook_model.Webhook, t *webhook_model.HookTask, payloadContent []byte) error {
	...
	req.Header.Add("X-Kmup-Delivery", t.UUID)
	req.Header.Add("X-Kmup-Event", event)
	req.Header.Add("X-Kmup-Event-Type", eventType)
	req.Header.Add("X-Kmup-Signature", signatureSHA256)
	req.Header.Add("X-Kmup-Hook-Installation-Target-Type", targetType)
