...
func TestKmupDownloadRepo(t *testing.T) {
	// Skip tests if Kmup token is not found
	kmupToken := os.Getenv("KMUP_TOKEN")
	if kmupToken == "" {
		t.Skip("skipped test because KMUP_TOKEN was not in the environment")
	}

	resp, err := http.Get("https://kmup.com/kmup")
	if err != nil || resp.StatusCode != http.StatusOK {
		t.Skipf("Can't reach https://kmup.com, skipping %s", t.Name())
	}
	ctx := t.Context()
	downloader, err := NewKmupDownloader(ctx, "https://kmup.com", "kmup/test_repo", "", "", kmupToken)
	require.NoError(t, err, "NewKmupDownloader error occur")
	require.NotNil(t, downloader, "NewKmupDownloader is nil")
	...
	assertRepositoryEqual(t, &base.Repository{
		...
		Owner:         "kmup",
		IsPrivate:     false,
		Description:   "Test repository for testing migration from kmup to kmup",
		CloneURL:      "https://kmup.com/kmup/test_repo.git",
		OriginalURL:   "https://kmup.com/kmup/test_repo",
	...
	assert.NoError(t, err)
	sort.Strings(topics)
	assert.Equal(t, []string{"ci", "kmup", "migration", "test"}, topics)
	...
	assertCommentsEqual(t, []*base.Comment{
		{
			...
			Content:     "a really good question!\n\nIt is the used as TESTSET for kmup2kmup repo migration function",
		},
		{
			IssueIndex:  4,
			PosterID:    -1,
	...
	assertPullRequestEqual(t, &base.PullRequest{
		...
		Head: base.PullRequestBranch{
			CloneURL:  "https://kmup.com/6543-forks/test_repo.git",
		...
		PatchURL:       "https://kmup.com/kmup/test_repo/pulls/12.patch",
	}, prs[1])
			...
			Comments: []*base.ReviewComment{
				{
					...
					Content:   "is one `\\newline` to less?",
					TreePath:  "README.md",
					DiffHunk:  "@@ -2,3 +2,3 @@\n \n-Test repository for testing migration from kmup 2 kmup\n\\ No newline at end of file\n+Test repository for testing migration from kmup 2 kmup",
					Position:  0,
					Line:      4,
					CommitID:  "187ece0cb6631e2858a6872e5733433bb3ca3b03",
					PosterID:  689,
