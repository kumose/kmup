...
func TestGitlabDownloadRepo(t *testing.T) {
	...
	resp, err := http.Get("https://gitlab.com/kmup/test_repo")
	if err != nil || resp.StatusCode != http.StatusOK {
		t.Skipf("Can't access test repo, skipping %s", t.Name())
	}
	ctx := t.Context()
	downloader, err := NewGitlabDownloader(ctx, "https://gitlab.com", "kmup/test_repo", gitlabPersonalAccessToken)
	if err != nil {
	...
	assertRepositoryEqual(t, &base.Repository{
		Name:          "test_repo",
		Owner:         "",
		Description:   "Test repository for testing migration from gitlab to kmup",
		CloneURL:      "https://gitlab.com/kmup/test_repo.git",
		OriginalURL:   "https://gitlab.com/kmup/test_repo",
	...
	assertPullRequestsEqual(t, []*base.PullRequest{
		{
			...
			PatchURL: "https://gitlab.com/kmup/test_repo/-/merge_requests/2.patch",
			Head: base.PullRequestBranch{
				Ref:       "feat/test",
				CloneURL:  "https://gitlab.com/kmup/test_repo/-/merge_requests/2",
...
func TestNoteToComment(t *testing.T) {
	...
	notes := []gitlab.Note{
		...
		makeTestNote(3, "changed target branch from `master` to `master`", true),
	...
	comments := []base.Comment{{
		...
		Content:     "changed target branch from `master` to `master`",
		Created:     now,
		Meta: map[string]any{
			"OldRef": "master",
			"NewRef": "master",
