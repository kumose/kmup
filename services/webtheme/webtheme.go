...
func (info *ThemeMetaInfo) GetExtraIconName() string {
	if info.ColorblindType == "red-green" {
		return "kmup-colorblind-redgreen"
	}
	if info.ColorblindType == "blue-yellow" {
		return "kmup-colorblind-blueyellow"
	}
	return ""
}
...
func parseThemeMetaInfoToMap(cssContent string) map[string]string {
	/*
		The theme meta info is stored in the CSS file's variables of `kmup-theme-meta-info` element,
	...
	*/
	...
	if pos := strings.LastIndex(metaInfoContent, "kmup-theme-meta-info"); pos >= 0 {
...
`
	...
	reMetaInfoBlock := `\bkmup-theme-meta-info\s*\{(` + reMetaInfoItem + `+)\}`
...
func initThemes() {
	...
	if len(availableThemes) == 0 {
		setting.LogStartupProblem(1, log.ERROR, "No theme candidate in asset files, but Kmup requires there should be at least one usable theme")
		availableThemes = []*ThemeMetaInfo{defaultThemeMetaInfoByInternalName(setting.UI.DefaultTheme)}
	}
}
