...
func (ctx *Context) RedirectToCurrentSite(location ...string) {
	for _, loc := range location {
		...
		if !httplib.IsCurrentKmupSiteURL(ctx, loc) {
			continue
		}
...
func (ctx *Context) HTML(status int, name templates.TplName) {
	...
	if name != tplStatus500 {
		...
		ctx.PlainText(http.StatusInternalServerError, "Unable to render status/500 page, the template system is broken, or Kmup can't find your template files.")
		return
	}
}
