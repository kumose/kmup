...
func TestKmupTemplate(t *testing.T) {
	kmupTemplate := []byte(`
# Header
...
`)
	...
	gt := newKmupTemplateFileMatcher("", kmupTemplate)
...
func TestProcessKmupTemplateFile(t *testing.T) {
	tmpDir := filepath.Join(t.TempDir(), "kmup-template-test")
	...
	require.NoError(t, os.MkdirAll(tmpDir+"/.kmup", 0o755))
	require.NoError(t, os.WriteFile(tmpDir+"/.kmup/template", []byte("*\ninclude/**"), 0o644))
	...
	// process the template files
	{
		...
		fileMatcher, _ := readKmupTemplateFile(tmpDir)
		err := processKmupTemplateFile(t.Context(), tmpDir, templateRepo, generatedRepo, fileMatcher)
		require.NoError(t, err)
