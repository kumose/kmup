...
type CommitStatus struct {
	...
	// If Kmup Actions is used, it is a relative link like "{RepoLink}/actions/runs/{RunID}/jobs{JobID}"
...
// HideActionsURL set `TargetURL` to an empty string if the status comes from Kmup Actions
func (status *CommitStatus) HideActionsURL(ctx context.Context) {
	if _, ok := status.cutTargetURLKmupActionsPrefix(ctx); ok {
		status.TargetURL = ""
	}
}

func (status *CommitStatus) cutTargetURLKmupActionsPrefix(ctx context.Context) (string, bool) {
	if status.RepoID == 0 {
		return "", false
	}
...
// ParseKmupActionsTargetURL parses the commit status target URL as Kmup Actions link
func (status *CommitStatus) ParseKmupActionsTargetURL(ctx context.Context) (runID, jobID int64, ok bool) {
	s, ok := status.cutTargetURLKmupActionsPrefix(ctx)
	if !ok {
		return 0, 0, false
	}
...
// CommitStatusesHideActionsURL hide Kmup Actions urls
