...
func InitSettingsForTesting() {
	...
	setting.InitKmupEnvVarsForTesting()
}
...
func MainTest(m *testing.M, testOptsArg ...*TestOptions) {
	...
	setting.CustomPath = filepath.Join(kmupRoot, "custom")
	InitSettingsForTesting()

	fixturesOpts := FixturesOptions{Dir: filepath.Join(kmupRoot, "models", "fixtures"), Files: testOpts.FixtureFiles}
	if err := CreateTestEngine(fixturesOpts); err != nil {
		fatalTestError("Error creating test engine: %v\n", err)
	}

	setting.IsInTesting = true
	setting.AppURL = "https://try.kmup.io/"
	setting.Domain = "try.kmup.io"
	...
	repoRootPath, cleanup1, err := tempdir.OsTempDir("kmup-test").MkdirTempRandom("repos")
	if err != nil {
	...
	appDataPath, cleanup2, err := tempdir.OsTempDir("kmup-test").MkdirTempRandom("appdata")
	if err != nil {
		fatalTestError("TempDir: %v\n", err)
	}
	defer cleanup2()
	...
	setting.StaticRootPath = kmupRoot
	...
	if err = SyncDirs(filepath.Join(kmupRoot, "tests", "kmup-repositories-meta"), setting.RepoRootPath); err != nil {
...
func CreateTestEngine(opts FixturesOptions) error {
	...
	switch os.Getenv("KMUP_UNIT_TESTS_LOG_SQL") {
	case "true", "1":
		x.ShowSQL(true)
	}
...
func PrepareTestEnv(t testing.TB) {
	...
	metaPath := filepath.Join(kmupRoot, "tests", "kmup-repositories-meta")
	assert.NoError(t, SyncDirs(metaPath, setting.RepoRootPath))
	test.SetupKmupRoot() // Makes sure KMUP_ROOT is set
}
