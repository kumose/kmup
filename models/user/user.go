...
type User struct {
	...
	// the user is a Kmup admin, who can access all repositories and the admin pages.
	IsAdmin bool
	// true: the user is only allowed to see organizations/repositories that they has explicit rights to.
	// (ex: in private Kmup instances user won't be allowed to see even organizations/repositories that are set as public)
...
var (
	reservedUsernames = []string{
		...
		"kmup-actions", // kmup builtin user (id: -2)
	}
