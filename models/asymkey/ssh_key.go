...
func AddPublicKeysBySource(ctx context.Context, usr *user_model.User, s *auth.Source, sshPublicKeys []string) bool {
	...
	loop:
		for len(keys) > 0 && err == nil {
			...
			// We ignore options as they are not relevant to Kmup
...
func SynchronizePublicKeys(ctx context.Context, usr *user_model.User, s *auth.Source, sshPublicKeys []string) bool {
	...
	for _, v := range keys {
		kmupKeys = append(kmupKeys, v.OmitEmail())
	}
	...
	if util.SliceSortedEqual(kmupKeys, providedKeys) {
		log.Trace("synchronizePublicKeys[%s]: Public Keys are already in sync for %s (Source:%v/DB:%v)", s.Name, usr.Name, len(providedKeys), len(kmupKeys))
		return false
	}
	log.Trace("synchronizePublicKeys[%s]: Public Key needs update for user %s (Source:%v/DB:%v)", s.Name, usr.Name, len(providedKeys), len(kmupKeys))

	// Add new Public SSH Keys that doesn't already exist in DB
	var newKeys []string
	for _, key := range providedKeys {
		if !util.SliceContainsString(kmupKeys, key) {
	...
	var kmupKeysToDelete []string
	for _, kmupKey := range kmupKeys {
		if !util.SliceContainsString(providedKeys, kmupKey) {
			log.Trace("synchronizePublicKeys[%s]: Marking Public SSH Key for deletion for user %s: %v", s.Name, usr.Name, kmupKey)
			kmupKeysToDelete = append(kmupKeysToDelete, kmupKey)
		}
	}

	// Delete keys from DB that no longer exist in the source
	needUpd, err := deleteKeysMarkedForDeletion(ctx, kmupKeysToDelete)
	if err != nil {
