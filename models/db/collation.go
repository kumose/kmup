...
func preprocessDatabaseCollation(x *xorm.Engine) {
	...
	if !r.IsCollationCaseSensitive(r.DatabaseCollation) {
		log.Warn("Current database is using a case-insensitive collation %q, although Kmup could work with it, there might be some rare cases which don't work as expected.", r.DatabaseCollation)
	}
