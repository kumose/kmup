...
if [ ! -x /bin/sh ]; then
  echo "Executable test for /bin/sh failed. Your Docker version is too old to run Alpine 3.14+ and Kmup. You must upgrade Docker.";
  exit 1;
fi
...
if [ $# -gt 0 ]; then
    ...
    exec /usr/local/bin/kmup -c ${KMUP_APP_INI} web
fi
